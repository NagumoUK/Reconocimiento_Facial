...
export default function FacialModal({ onClose, onSuccess }: FacialModalProps) {
  ...
  const runRealtimeLoginDetection = () => {
    ...
    setTimeout(() => {
      ...
      if (facialUsers.length > 0) {
        ...
        setStatus('error');
        setMessage('No hay rostros registrados. Por favor, regístrese primero.');
      }
    }, 2500);
  };
  ...
  const handleSaveRegistration = () => {
    ...
    const newUserData = {
      ...
      avatar: capturedImage || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150'
    };
